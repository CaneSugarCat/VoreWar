...
public class RaceEditorPanel : MonoBehaviour
{
    ...
    bool SubRaceLoaded = false;
    int subracedropdownindex;
    ...
    public void SubRaceChanged()
    {
        ...
        {
            if (SubRaceDropdown.value == 0)
            {
                LoadRace();
            }
            else 
            {
                LoadSubRace(race, SubRaceDropdown.value - 1);
            }
            UpdateInteractable();
        }
    }
    ...
    internal void SaveRace()
    {
        try
        {
            if (PreviousRace != (Race)(-1) && !SubRaceLoaded)
            {
    ...
    internal void LoadRace()
    {
        if (Enum.TryParse(RaceDropdown.options[RaceDropdown.value].text, out Race race))
        {
            ...
            if (State.SubRaces.ContainsKey(race) && !SubRaceLoaded)
            {
    ...
    internal void LoadSubRace(Race superrace, int index)
    {
    ...
    public void CreateSubrace()
    {
        if (Enum.TryParse(RaceDropdown.options[RaceDropdown.value].text, out Race race))
        {
            SubRaceParameters.CreateSubRaceParameters(race, SubRaceName.text);
            CancelSubrace();
            if (SubRaceLoaded)
            {
                SaveSubRace();
    ...
    public void ActivateGeneral()
    {
        ...
        SubRaceButton.interactable = true;
    }
    ...
    public void ActivateSubracePanel()
    {
        GeneralPanel.SetActive(false);
        TraitsPanel.SetActive(false);
        UnitTagsPanel.SetActive(false);
        ...
        TraitsButton.interactable = true;
        UnitTagsButton.interactable = true;
        SubRaceButton.interactable = false;
    }
}
